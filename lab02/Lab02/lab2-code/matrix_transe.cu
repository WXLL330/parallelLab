...
int main()
{
    ...
    float gElapsedTimes, gElapsedTime, cElapsedTime;
    ...
    CHECK(cudaEventRecord(stop,0));
    CHECK(cudaEventSynchronize(stop));
    CHECK(cudaEventElapsedTime(&gElapsedTimes,start,stop));
    ...
    fprintf(output, "%.2f,%.2f,%.2f\n", cElapsedTime, gElapsedTime, gElapsedTimes);
    fclose(output);
